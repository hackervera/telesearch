...
var listen = function(address, privkey, pubkey){
    var self = this;
    self.privkey = fs.readFileSync(privkey,'utf8');
    self.pubkey = fs.readFileSync(pubkey,'utf8');
    ...
    s.on("+confirm", function(remoteipp, telex, line) {
        if(telex["+confirm"] == address){
            var pass = dcrypt.verify.createVerify('sha1')
            .update(telex["message"])
            .verify(options.key, telex["signature"], 'hex');
            if(pass){
                self.emit("confirm", telex);
            }
        }
    });
}
